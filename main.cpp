...
struct Board {
  ...
  uint8_t castle_white_kingside : 1;
  uint8_t castle_white_queenside : 1;
...
bool parse_blocks(const std::string& fen,
                 std::string& board,
                 std::string& turn,
                 std::string& castle,
...
bool parse_move_count(Board& board, const std::string& half_move, const std::string& full_move) {
    try {
...
Board import_fen(const std::string& fen) {
    Board board;
...
int main(int argc, char *argv[]) {
    ...
    Board board = import_fen(read_file(argv[1]));
